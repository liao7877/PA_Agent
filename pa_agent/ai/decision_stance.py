...
def build_decision_stance_guidance(stance: str | None) -> str:
    ...
    common_rules = (
        ...
        "- **盈亏比由结构自然得出，不设固定上限**：止盈取图表结构位；强趋势可取 2R+（若结构支持），"
        "震荡/弱信号可保守至 1R 附近；**最低要求**回报>风险（>1:1），且交易者方程为正，"
        "否则程序拒单。\n"
        ...
        "- **§9.0P 背景限价**：当 §9.0=否（无已收盘信号棒）时，**必须**评估 §9.0P；"
        "§9.0P=是 时可输出限价单，signal_bar.quality 可为 weak/invalid。\n"
        "- **direction=neutral 约束**：neutral 时 §9.0/§9.0P **仅允许顺 §2.4**（AIL→只做多侧，"
    ...
    if normalized == "conservative":
        profile = (
            ...
            "§9.0 可判「是」并输出限价单（非一律观望）；§9.0=否 时仍应评估 §9.0P。"
            "中部位置或无结构锚点时才默认 wait。\n"
            "- §10：止损必须明确且不过大；10.3 交易者方程边际情况倾向判「否」。"
            "止盈以结构位为准，盈亏比不设上限，底线回报>风险且方程为正。\n"
            "- §14：从严扫描；有疑虑即不下单。\n"
        ...
        profile = (
            ...
            "盈亏比由结构决定（底线 >1:1），但须在 trade_confidence_reasoning 写明假设。\n"
            "- §14：仅明确触犯才不下单；不要因「不够完美」单独放弃。\n"
            "- trade_confidence：35–49 且入场逻辑完整时，可给出下单方案（在 reasoning 说明风险克制）。\n"
            "- 顺势通道/尖峰延续、区间边界反弹：方向与周期一致时可优先考虑下单，而非默认等待。\n"
        )
        ...
        profile = (
            ...
            "可判「是」；盈亏比由结构决定（底线 >1:1），必须在 reasoning 强调风险克制。\n"
        ...
        profile = (
            ...
            "盈亏比由结构决定（底线 >1:1），且胜率×回报>败率×风险，否则**必须**判「否」并走不下单路径，"
