...
- **盈亏比由结构自然得出，不设固定上限**：止盈取图表上最近有结构意义的价位（支撑/阻力、通道边界、测量移动、区间对侧等）；强趋势常见 2R 及以上（若结构支持），震荡/弱信号可保守至 1R 附近
- **最低要求**：回报 > 风险（> 1:1）；且须满足 **胜率%×回报 > (100−胜率)%×风险**（交易者方程为正）。目标过远导致方程不通过时应 reject 或改用更近的结构目标，**禁止**为通过校验而压低盈亏比或编造图上不存在的远目标
...
- `take_profit_price` 不应放在当前 K1 区间内部、最近噪音位、或仅为凑方程而选择的微小目标；除非该点位本身是明确反向结构边界，否则应选择下一层有效阻力 / 支撑 / 通道边界。
- 若使用更合理的结构止损后，最近有效结构目标导致 RR ≤ 1.0，或方程不通过，则不得缩窄 stop/target 来凑方程，应 `order_type=不下单`，terminal 指向 10.3 或更早失败节点。
